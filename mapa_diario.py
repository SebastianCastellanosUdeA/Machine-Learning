...
barrios = gpd.read_file("blz.gpkg")
...
for _, row in combinaciones_unicas.iterrows():
    ...
    datos_dia.plot(column='IVS', ax=ax, legend=True, cmap='OrRd',
                   vmin=0, vmax=1,  # Fijar los valores mínimo y máximo de la escala de colores
    #ax.set_title(f'PM2.5 {fecha.strftime("%Y-%m-%d")}')
                   legend_kwds={'label': "IVS", 'orientation': "horizontal"})
    ax.set_title(f'IVS {ano} - {mes}')
    ...
    plt.savefig(f'IVS_{ano}_{mes}.png')
    ...
    # Mapa para PM2.5
    datos_dia.plot(column='PM', ax=axs[0], legend=True, cmap='OrRd',
                    vmin=6, vmax=18, 
    ...
    # Mapa para Locais
    datos_dia.plot(column='locais_sc', ax=axs[2], legend=True, cmap='Greens',
                    vmin=0, vmax=120,
    ...
    axs[2].set_xlabel('')
    axs[2].set_ylabel('')
    plt.savefig(f'mapa_composto_{ano}_{mes}.png')
    plt.close()
