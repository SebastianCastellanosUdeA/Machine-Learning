...
# Guardar las métricas de RMSE para cada pliegue
rmse_scores = []
fold_test_indices = {}
kf1 = KFold(n_splits=n_splits, shuffle=True, random_state=42)

for fold, (train_index, test_index) in enumerate(kf1.split(X_trn0)):
    print(f"Fold {fold + 1}")
    # Dividir el conjunto de datos en entrenamiento y test según las particiones de KFold
    X_trn, X_tst = X_trn0.iloc[train_index], X_trn0.iloc[test_index]
    y_trn, y_tst = y_trn0.iloc[train_index], y_trn0.iloc[test_index]
    ...
    # Definindo o modulo com a métrica RMSE
    module = SimpleModule.regression(model1,optimizer=optimizer, metrics={'rmse': MeanSquaredError(squared=False)})
...
for idx in range(len(X_trn0)):
    # Si el índice no está en atypical_indices, agregar la fila a X_train_f
    if idx not in atypical_indices:
        filas_a_agregar.append(X_trn0.iloc[idx])
        valores_a_agregar.append(y_trn0.iloc[idx])
        
#MODELO FINAL
X_train_f = pd.DataFrame(filas_a_agregar, columns=X_trn0.columns)
y_train_f = pd.Series(valores_a_agregar, name=y_trn0.name)
